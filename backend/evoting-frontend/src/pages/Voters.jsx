        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left text-sm text-slate-400">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? <tr><td colSpan="5" className="px-6 py-20 text-center"><Loader2 className="animate-spin inline" /> Loading...</td></tr> :
                filteredVoters.map((v) => (
                  <tr key={v.ID} className="group hover:bg-slate-800/30 transition-all duration-200">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm bg-gradient-to-br from-indigo-500/10 to-purple-500/10 text-indigo-400 border border-indigo-500/20">{v.FullName.charAt(0)}</div>
                        <div><div className="font-semibold text-slate-200">{v.FullName}</div><div className="text-xs text-slate-500 font-mono">ID: {v.VoterID}</div></div>
                      </div>
                    </td>
                    <td className="px-6 py-4"><div className="flex items-center gap-2"><Phone size={12} /><span className="font-mono">{v.Mobile}</span></div></td>
                    <td className="px-6 py-4">
                      <button
                    ...
                    <td className="px-6 py-4 text-right relative">
                      <button onClick={() => setActiveDropdown(activeDropdown === v.ID ? null : v.ID)} className="p-2 rounded-lg text-slate-500 hover:text-white"><MoreVertical size={18} /></button>
                      {activeDropdown === v.ID && (
                        <div ref={dropdownRef} className="absolute right-10 top-10 w-48 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 p-1.5">
